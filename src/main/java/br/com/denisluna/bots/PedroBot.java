...
import br.com.denisluna.telegrambots.utils.PadraoDeArquivos;
...
public class PedroBot extends Bot {
	...
	public void responde(Message mensagem) {
		...
		if (mensagem.getMessageType().equals(PadraoDeTags.ENTITIES)) {
			...
			this.telegram.sendVoice(this.getChatId(), PadraoDeArquivos.BAN);
			this.telegram.sendMessage(this.getChatId(), resposta);
		} else {
			...
			if (mensagem.getText().contains("PORRA") && mensagem.getText().contains(this.getNomeBot().toUpperCase())) {
				...
				this.getTelegram().sendPhoto(this.getChatId(), PadraoDeArquivos.LOVINHO_GRANDE, "");
			} else if (mensagem.getText().contains("UHULL")) {
				this.getTelegram().sendPhoto(this.getChatId(), PadraoDeArquivos.UHULL, "");
				...
				this.telegram.sendVoice(this.getChatId(), PadraoDeArquivos.CU_RASPAGEM);
				this.telegram.sendMessage(this.getChatId(), resposta);
				...
				this.telegram.sendVoice(this.getChatId(), PadraoDeArquivos.LUCIO);
				this.telegram.sendMessage(this.getChatId(), resposta);
				...
				if (mensagem.getText().contains("MORRA")) {
					...
					resposta.add("Aí, posso morrer não " + mensagem.getUsuarioFrom().getNome()
